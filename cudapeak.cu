...
__global__ void compute_sp_sincos_fpu_1_8(float *ptr);
__global__ void compute_sp_sincos_fpu_1_4(float *ptr);
__global__ void compute_sp_sincos_fpu_1_2(float *ptr);
__global__ void compute_sp_sincos_fpu_1_1(float *ptr);
__global__ void compute_sp_sincos_fpu_2_1(float *ptr);
__global__ void compute_sp_sincos_fpu_4_1(float *ptr);
__global__ void compute_sp_sincos_fpu_8_1(float *ptr);
__global__ void compute_sp_sincos_fpu_16_1(float *ptr);
__global__ void compute_sp_sincos_fpu_32_1(float *ptr);
__global__ void compute_sp_sincos_fpu_64_1(float *ptr);
__global__ void compute_sp_sincos_fpu_128_1(float *ptr);
__global__ void compute_sp_sincos_sfu_1_8(float *ptr);
__global__ void compute_sp_sincos_sfu_1_4(float *ptr);
__global__ void compute_sp_sincos_sfu_1_2(float *ptr);
__global__ void compute_sp_sincos_sfu_1_1(float *ptr);
__global__ void compute_sp_sincos_sfu_2_1(float *ptr);
__global__ void compute_sp_sincos_sfu_4_1(float *ptr);
__global__ void compute_sp_sincos_sfu_8_1(float *ptr);
__global__ void compute_sp_sincos_sfu_16_1(float *ptr);
__global__ void compute_sp_sincos_sfu_32_1(float *ptr);
__global__ void compute_sp_sincos_sfu_64_1(float *ptr);
__global__ void compute_sp_sincos_sfu_128_1(float *ptr);
...
void run_compute_sp_sincos_fpu() {
    ...
    // Run kernels
    double milliseconds;
    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_1_8, ptr, gridDim, blockDim);
    report("fma:sincos (fpu) ->    1:8", milliseconds, gflops, gbytes, gflops*8);

    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_1_4, ptr, gridDim, blockDim);
    report("fma:sincos (fpu) ->    1:4", milliseconds, gflops, gbytes, gflops*4);

    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_1_2, ptr, gridDim, blockDim);
    report("fma:sincos (fpu) ->    1:2", milliseconds, gflops, gbytes, gflops*2);

    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_1_1, ptr, gridDim, blockDim);
    report("fma:sincos (fpu) ->    1:1", milliseconds, gflops, gbytes, gflops*1);

    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_2_1, ptr, gridDim, blockDim);
    report("fma:sincos (fpu) ->    2:1", milliseconds, gflops, gbytes, gflops/2);

    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_4_1, ptr, gridDim, blockDim);
    report("fma:sincos (fpu) ->    4:1", milliseconds, gflops, gbytes, gflops/4);

    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_8_1, ptr, gridDim, blockDim);
    report("fma:sincos (fpu) ->    8:1", milliseconds, gflops, gbytes, gflops/8);

    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_16_1, ptr, gridDim, blockDim);
    report("fma:sincos (fpu) ->   16:1", milliseconds, gflops, gbytes, gflops/16);

    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_32_1, ptr, gridDim, blockDim);
    report("fma:sincos (fpu) ->   32:1", milliseconds, gflops, gbytes, gflops/32);

    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_64_1, ptr, gridDim, blockDim);
    report("fma:sincos (fpu) ->   64:1", milliseconds, gflops, gbytes, gflops/64);

    milliseconds = run_kernel((void *) &compute_sp_sincos_fpu_128_1, ptr, gridDim, blockDim);
...
void run_compute_sp_sincos_sfu() {
    ...
    // Run kernels
    double milliseconds;
    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_1_8, ptr, gridDim, blockDim);
    report("fma:sincos (sfu) ->    1:8", milliseconds, gflops, gbytes, gflops*8);

    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_1_4, ptr, gridDim, blockDim);
    report("fma:sincos (sfu) ->    1:4", milliseconds, gflops, gbytes, gflops*4);

    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_1_2, ptr, gridDim, blockDim);
    report("fma:sincos (sfu) ->    1:2", milliseconds, gflops, gbytes, gflops*2);

    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_1_1, ptr, gridDim, blockDim);
    report("fma:sincos (sfu) ->    1:1", milliseconds, gflops, gbytes, gflops*1);

    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_2_1, ptr, gridDim, blockDim);
    report("fma:sincos (sfu) ->    2:1", milliseconds, gflops, gbytes, gflops/2);

    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_4_1, ptr, gridDim, blockDim);
    report("fma:sincos (sfu) ->    4:1", milliseconds, gflops, gbytes, gflops/4);

    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_8_1, ptr, gridDim, blockDim);
    report("fma:sincos (sfu) ->    8:1", milliseconds, gflops, gbytes, gflops/8);

    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_16_1, ptr, gridDim, blockDim);
    report("fma:sincos (sfu) ->   16:1", milliseconds, gflops, gbytes, gflops/16);

    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_32_1, ptr, gridDim, blockDim);
    report("fma:sincos (sfu) ->   32:1", milliseconds, gflops, gbytes, gflops/32);

    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_64_1, ptr, gridDim, blockDim);
    report("fma:sincos (sfu) ->   64:1", milliseconds, gflops, gbytes, gflops/64);

    milliseconds = run_kernel((void *) &compute_sp_sincos_sfu_128_1, ptr, gridDim, blockDim);
